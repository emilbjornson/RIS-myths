...
plot(Nvalues*A,SNR_DFrelay,'k--','LineWidth',2);
plot(Nvalues*A,SNR_RIS,'b-','LineWidth',2);
set(gca,'XScale','log');
set(gca,'YScale','log');
xlabel('Surface area [m$^2$]','Interpreter','Latex');
ylabel('End-to-end SNR','Interpreter','Latex');
legend({'DF relay','RIS'},'Interpreter','Latex','Location','SouthEast');
set(gca,'fontsize',18);
xlim([1e-3 1e3]);
